...
const projectionBuilder = (attributes, table, entity, type = false) => {
    ...
    for (const attribute of attrs) {
        ...
        if (typeof attribute === 'string') {
            ...
            const attr = (0, checkAttribute_js_1.default)(attribute, entity ? table[entity].schema.attributes : table.Table.attributes);
            ...
            for (const entity in attribute) {
                ...
                if (table[entity]) {
                        ...
                        : // support simple string list
                            typeof attribute[entity] === 'string'
                                ? String(attribute[entity])
                                    .split(',')
                                    .map(x => x.trim())
                                : (0, utils_js_1.error)(`Only arrays or strings are supported`);
                    // Loop entity projections
                    for (const ent_attribute of ent_attrs) {
                        // Check for string type
                        if (typeof ent_attribute != 'string')
                            (0, utils_js_1.error)(`Entity projections must be string values`);
                        // Check the attribute and merge results
                        const attr = (0, checkAttribute_js_1.default)(ent_attribute, table[entity].schema.attributes);
                        if (!Object.values(names).includes(attr)) {
                            names[`#proj${++index}`] = attr;
                        }
                    ...
                    (0, utils_js_1.error)(`'${entity}' is not a valid entity on this table`);
                }
            }
            // Throw error if invalid type
        }
        else {
            (0, utils_js_1.error)(`'${typeof attribute}' is an invalid type. Projections require strings or arrays`);
        }
    }
    return {
        names,
