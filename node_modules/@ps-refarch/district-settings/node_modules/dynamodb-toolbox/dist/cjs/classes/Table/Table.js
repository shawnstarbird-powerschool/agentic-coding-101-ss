...
const parseTable_js_1 = require("../../lib/parseTable.js");
...
const Entity_js_1 = __importDefault(require("../Entity/Entity.js"));
...
class Table {
    ...
    constructor(table) {
        ...
        if (typeof table !== 'object' || Array.isArray(table)) {
            (0, utils_js_1.error)('Please provide a valid table definition');
        }
        // Parse the table and merge into this
        Object.assign(this, (0, parseTable_js_1.parseTable)(table));
    }
    ...
    set DocumentClient(docClient) {
        ...
        if (docClient && docClient.send) {
            ...
            (0, utils_js_1.error)('Invalid DocumentClient');
        }
    }
    /**
    ...
    addEntity(entity) {
        ...
        for (const i in entities) {
            ...
            if (!(entity instanceof Entity_js_1.default)) {
                (0, utils_js_1.error)('Invalid Entity');
            }
            ...
            if (reservedWords.includes(entity.name)) {
                (0, utils_js_1.error)(`'${entity.name}' is a reserved word and cannot be used to name an Entity`);
            }
            // Check for sortKeys (if applicable)
            if (!this.Table.sortKey && entity.schema.keys.sortKey) {
                (0, utils_js_1.error)(`${entity.name} entity contains a sortKey, but the Table does not`);
            }
            else if (this.Table.sortKey && !entity.schema.keys.sortKey) {
                (0, utils_js_1.error)(`${entity.name} entity does not have a sortKey defined`);
            }
                ...
                switch (key) {
                    ...
                    case 'sortKey':
                        ...
                        if (attr !== this.Table[key] && this.Table[key]) {
                            ...
                            if (!entity.schema.attributes[this.Table[key]]) {
                                ...
                                (0, utils_js_1.error)(`The Table's ${key} name (${String(this.Table[key])}) conflicts with an Entity attribute name`);
                            }
                        }
                        break;
                    ...
                    default:
                        ...
                        if (!this.Table.indexes[key])
                            (0, utils_js_1.error)(`'${key}' is not a valid secondary index name`);
                        ...
                        for (const keyType in attr) {
                            ...
                            if (!this.Table.indexes[key][keyType]) {
                                (0, utils_js_1.error)(`${entity.name} contains a ${keyType}, but it is not used by ${key}`);
                            }
                            ...
                            if (attr[keyType] !== this.Table.indexes[key][keyType]) {
                                ...
                                if (!entity.schema.attributes[this.Table.indexes[key][keyType]]) {
                                    ...
                                    if (entity.schema.attributes[attr[keyType]].map &&
                                        ...
                                        (0, utils_js_1.error)(`${key}'s ${keyType} cannot map to the '${attr[keyType]}' alias because it is already mapped to another table attribute`);
                                    }
                        ...
                        if (this.Table.indexes[key].partitionKey &&
                            ...
                            (0, utils_js_1.error)(`${key} requires mappings for both the partitionKey and the sortKey`);
                        }
                        break;
                }
            }
            ...
            for (const attr in entity.schema.attributes) {
                ...
                if (this.Table.entityField &&
                    ...
                    (0, utils_js_1.error)(`Attribute or alias '${attr}' conflicts with the table's 'entityField' mapping or entity alias`);
                    ...
                    if (this.Table.attributes[attr].type &&
                        ...
                        (0, utils_js_1.error)(`${entity.name} attribute type for '${attr}' (${entity.schema.attributes[attr].type}) does not match table's type (${this.Table.attributes[attr].type})`);
                    }
                    // Add entity mappings
    ...
    queryParams(pk, options = {}, params = {}, projections = false) {
        ...
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid query options: ${args.join(', ')}`);
        // Verify pk
        if ((typeof pk !== 'string' && typeof pk !== 'number') ||
            (typeof pk === 'string' && pk.trim().length === 0)) {
            (0, utils_js_1.error)(`Query requires a string, number or binary 'partitionKey' as its first parameter`);
        }
        // Verify index
        if (index !== undefined && !this.Table.indexes[index]) {
            (0, utils_js_1.error)(`'${index}' is not a valid index name`);
        }
        // Verify limit
        if (limit !== undefined && (!Number.isInteger(limit) || limit < 0)) {
            (0, utils_js_1.error)(`'limit' must be a positive integer`);
        }
        // Verify reverse
        if (reverse !== undefined && typeof reverse !== 'boolean')
            (0, utils_js_1.error)(`'reverse' requires a boolean`);
        ...
        if (select !== undefined &&
            ...
            (0, utils_js_1.error)(`'select' must be one of 'ALL_ATTRIBUTES', 'ALL_PROJECTED_ATTRIBUTES', 'SPECIFIC_ATTRIBUTES', OR 'COUNT'`);
        }
        // Verify entity
        if (entity !== undefined && (typeof entity !== 'string' || !(entity in this))) {
            (0, utils_js_1.error)(`'entity' must be a string and a valid table Entity name`);
        }
        // Verify capacity
        ...
        if (startKey && (typeof startKey !== 'object' || Array.isArray(startKey))) {
            (0, utils_js_1.error)(`'startKey' requires a valid object`);
        }
        ...
        if (lt !== undefined) {
            value = value ? (0, utils_js_1.conditionError)(f) : lt;
            f = 'lt';
            operator = '<';
        }
        if (lte !== undefined) {
            value = value ? (0, utils_js_1.conditionError)(f) : lte;
            f = 'lte';
            operator = '<=';
        }
        if (gt !== undefined) {
            value = value ? (0, utils_js_1.conditionError)(f) : gt;
            f = 'gt';
            operator = '>';
        }
        if (gte !== undefined) {
            value = value ? (0, utils_js_1.conditionError)(f) : gte;
            f = 'gte';
            operator = '>=';
        }
        if (beginsWith !== undefined) {
            value = value ? (0, utils_js_1.conditionError)(f) : beginsWith;
            f = 'beginsWith';
            operator = 'BEGINS_WITH';
        }
        if (between !== undefined) {
            value = value ? (0, utils_js_1.conditionError)(f) : between;
            f = 'between';
            operator = 'BETWEEN';
        }
        ...
        if (operator) {
            ...
            const sk = index
                ? this.Table.indexes[index].sortKey
                    ...
                    : (0, utils_js_1.error)(`Conditional expressions require the index to have a sortKey`)
                : this.Table.sortKey
                    ? this.Table.attributes[this.Table.sortKey]
                    : (0, utils_js_1.error)(`Conditional expressions require the table to have a sortKey`);
            // Init validateType
            ...
            if (operator === 'BETWEEN') {
                ...
                if (!Array.isArray(value) || value.length !== 2) {
                    (0, utils_js_1.error)(`'between' conditions requires an array with two values.`);
                }
        ...
        if (filters) {
            // Parse the filter
            const { expression, names, values } = (0, expressionBuilder_js_1.default)(filters, this, entity);
        ...
        // If projections
        if (attributes) {
            const { names, projections, entities, tableAttrs } = (0, projectionBuilder_js_1.default)(attributes, this, entity, true);
            if (Object.keys(names).length > 0) {
    ...
    scanParams(options = {}, params = {}, meta = false) {
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid scan options: ${args.join(', ')}`);
        // Verify index
        if (index !== undefined && !this.Table.indexes[index]) {
            (0, utils_js_1.error)(`'${index}' is not a valid index name`);
        }
        ...
        if (segments !== undefined && (!Number.isInteger(segments) || segments < 1)) {
            (0, utils_js_1.error)(`'segments' must be an integer greater than 1`);
        }
        if (segment !== undefined &&
            (!Number.isInteger(segment) || segment < 0 || segment >= segments)) {
            (0, utils_js_1.error)(`'segment' must be an integer greater than or equal to 0 and less than the total number of segments`);
        }
        if ((segments !== undefined && segment === undefined) ||
            (segments === undefined && segment !== undefined)) {
            (0, utils_js_1.error)(`Both 'segments' and 'segment' must be provided`);
        }
        // Default names and values
        let ExpressionAttributeNames = {};
    ...
    batchGetParams(_items, options = {}, params = {}, meta = false) {
        ...
        // Error on no items
        if (items.length === 0)
            (0, utils_js_1.error)(`No items supplied`);
        const { capacity, consistent, attributes, ..._args } = options;
        // Remove other valid options from options
        const args = Object.keys(_args).filter(x => !['execute', 'parse'].includes(x));
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid batchGet options: ${args.join(', ')}`);
        // Verify capacity
        if (capacity !== undefined &&
            (typeof capacity !== 'string' ||
                !['NONE', 'TOTAL', 'INDEXES'].includes(capacity.toUpperCase()))) {
        ...
        for (const i in items) {
            ...
            if (item && item.Table && item.Table.Table && item.Key && ((_a = item.Key) === null || _a === void 0 ? void 0 : _a.constructor) === Object) {
                ...
                (0, utils_js_1.error)(`Item references must contain a valid Table object and Key`);
            }
        }
                ...
                for (const tbl in consistent) {
                    ...
                    if (RequestItems[tbl_name]) {
                        if (typeof consistent[tbl] === 'boolean') {
                            ...
                            (0, utils_js_1.error)(`'consistent' values must be booleans (${tbl})`);
                        }
                    }
                    else {
                        (0, utils_js_1.error)(`There are no items for the table or table alias: ${tbl}`);
                    }
                }
            }
            else {
                (0, utils_js_1.error)(`'consistent' must be a boolean or an map of table names`);
            }
        }
        // If projections
        if (attributes) {
            ...
            if (Array.isArray(attributes)) {
                if (Object.keys(RequestItems).length === 1) {
                    ...
                    (0, utils_js_1.error)(`'attributes' must use a table map when requesting items from multiple tables`);
                }
            }
            for (const tbl in attrs) {
                const tbl_name = TableAliases[tbl] || tbl;
                if (Tables[tbl_name]) {
                    const { names, projections, entities, tableAttrs } = (0, projectionBuilder_js_1.default)(attrs[tbl], Tables[tbl_name], null, true);
                    ...
                    (0, utils_js_1.error)(`There are no items for the table: ${tbl}`);
                }
            }
        }
    ...
    batchWriteParams(_items, options = {}, params = {}, meta = false) {
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid batchWrite options: ${args.join(', ')}`);
        // Verify capacity
        if (capacity !== undefined &&
            (typeof capacity !== 'string' ||
                !['NONE', 'TOTAL', 'INDEXES'].includes(capacity.toUpperCase()))) {
    ...
    transactGetParams(_items, options = {}, meta = false) {
        ...
        const args = Object.keys(_args).filter(x => !['execute', 'parse'].includes(x));
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid transactGet options: ${args.join(', ')}`);
        if (capacity !== undefined &&
            (typeof capacity !== 'string' ||
                !['NONE', 'TOTAL', 'INDEXES'].includes(capacity.toUpperCase()))) {
            (0, utils_js_1.error)(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
        ...
        const payload = Object.assign({
            ...
            TransactItems: items.map(item => {
                ...
                if (!('Get' in _item) || Object.keys(_item).length > 1) {
                    (0, utils_js_1.error)(`Invalid transaction item. Use the 'getTransaction' method on an entity.`);
                }
                return _item;
            }),
    ...
    transactWriteParams(_items, options = {}, params = {}) {
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid transactWrite options: ${args.join(', ')}`);
        // Verify capacity
        if (capacity !== undefined &&
            (typeof capacity !== 'string' ||
                !['NONE', 'TOTAL', 'INDEXES'].includes(capacity.toUpperCase()))) {
        ...
        if (token !== undefined &&
            ...
            (0, utils_js_1.error)(`'token' must be a string up to 36 characters long `);
        }
        // Generate the payload
        const payload = Object.assign({
            // Loop through items
            TransactItems: items.map(item => {
                ...
                (!('ConditionCheck' in item) &&
                    ...
                    (0, utils_js_1.error)(`Invalid transaction item. Use the 'putTransaction', 'updateTransaction', 'deleteTransaction', or 'conditionCheck' methods on an entity.`);
                }
                return item;
            }),
    ...
    async parse(entity, input, include = []) {
        if (!this[entity])
            (0, utils_js_1.error)(`'${entity}' is not a valid Entity`);
