...
const cloneDeep = __importStar(require("deep-copy"));
const parseEntity_js_1 = __importDefault(require("../../lib/parseEntity.js"));
const validateTypes_js_1 = __importDefault(require("../../lib/validateTypes.js"));
const normalizeData_js_1 = __importDefault(require("../../lib/normalizeData.js"));
const formatItem_js_1 = __importDefault(require("../../lib/formatItem.js"));
const getKey_js_1 = __importDefault(require("../../lib/getKey.js"));
const expressionBuilder_js_1 = __importDefault(require("../../lib/expressionBuilder.js"));
const projectionBuilder_js_1 = __importDefault(require("../../lib/projectionBuilder.js"));
...
class Entity {
    ...
    constructor(entity) {
        ...
        if ((entity === null || entity === void 0 ? void 0 : entity.constructor) !== Object) {
            (0, utils_js_1.error)('Please provide a valid entity definition');
        }
        ...
        Object.assign(this, (0, parseEntity_js_1.default)(entity));
    }
    get table() {
        return this._table;
    }
    ...
    get DocumentClient() {
        ...
        if ((_a = this.table) === null || _a === void 0 ? void 0 : _a.DocumentClient) {
            ...
            return (0, utils_js_1.error)('DocumentClient required for this operation');
        }
    }
    ...
    get partitionKey() {
        return this.schema.keys.partitionKey
            ...
            : (0, utils_js_1.error)(`No partitionKey defined`);
    }
    ...
    attribute(attr) {
        return this.schema.attributes[attr] && this.schema.attributes[attr].map
            ...
            : this.schema.attributes[attr]
                ? attr
                : (0, utils_js_1.error)(`'${attr}' does not exist or is an invalid alias`);
    }
    parse(input, include = []) {
        ...
        if (Array.isArray(data)) {
            return data.map(item => (0, formatItem_js_1.default)()(schema.attributes, linked, item, include));
        }
        else {
            return (0, formatItem_js_1.default)()(schema.attributes, linked, data, include);
        }
    }
    /**
    ...
    getTransaction(item, options = {}) {
        ...
        if (Object.keys(args).length > 0) {
            (0, utils_js_1.error)(`Invalid get transaction options: ${Object.keys(args).join(', ')}`);
        }
    ...
    getParams(item, options = {}, params = {}) {
        ...
        const data = (0, normalizeData_js_1.default)()(schema.attributes, linked, Object.assign({}, defaults, item), true, false);
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid get options: ${args.join(', ')}`);
        // Verify consistent read
        if (consistent !== undefined && typeof consistent !== 'boolean') {
            (0, utils_js_1.error)(`'consistent' requires a boolean`);
        }
        // Verify capacity
        if (capacity !== undefined &&
            (typeof capacity !== 'string' ||
                !['NONE', 'TOTAL', 'INDEXES'].includes(capacity.toUpperCase()))) {
            (0, utils_js_1.error)(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
        }
        let ExpressionAttributeNames; // init ExpressionAttributeNames
        let ProjectionExpression; // init ProjectionExpression
        // If projections
        if (attributes) {
            const { names, projections } = (0, projectionBuilder_js_1.default)(attributes, this.table, this.name);
        ...
        const payload = Object.assign({
            ...
            Key: (0, getKey_js_1.default)()(data, schema.attributes, schema.keys.partitionKey, schema.keys.sortKey),
    ...
    deleteTransaction(item, options = {}, params) {
        ...
        // Error on extraneous arguments
        if (Object.keys(args).length > 0) {
            (0, utils_js_1.error)(`Invalid delete transaction options: ${Object.keys(args).join(', ')}`);
        }
    ...
    deleteParams(item, options = {}, params = {}) {
        // Extract schema and merge defaults
        const { schema, defaults, linked, _table } = this;
        const data = (0, normalizeData_js_1.default)()(schema.attributes, linked, Object.assign({}, defaults, item), true);
        const { conditions, // ConditionExpression
        capacity, // ReturnConsumedCapacity (none, total, or indexes)
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid delete options: ${args.join(', ')}`);
        // Verify metrics
        if (metrics !== undefined &&
            (typeof metrics !== 'string' || !['NONE', 'SIZE'].includes(metrics.toUpperCase()))) {
            (0, utils_js_1.error)(`'metrics' must be one of 'NONE' OR 'SIZE'`);
        }
        // Verify capacity
        if (capacity !== undefined &&
            (typeof capacity !== 'string' ||
        ...
        if (returnValues !== undefined &&
            ...
            (0, utils_js_1.error)(`'returnValues' must be one of 'NONE' OR 'ALL_OLD'`);
        }
        let ExpressionAttributeNames; // init ExpressionAttributeNames
        ...
        if (conditions) {
            ...
            const { expression, names, values } = (0, expressionBuilder_js_1.default)(conditions, this.table, this.name);
            if (Object.keys(names).length > 0) {
        ...
        const payload = Object.assign({
            TableName: _table.name,
            Key: (0, getKey_js_1.default)()(data, schema.attributes, schema.keys.partitionKey, schema.keys.sortKey),
        }, ExpressionAttributeNames ? { ExpressionAttributeNames } : null, !(0, utils_js_1.isEmpty)(ExpressionAttributeValues) ? { ExpressionAttributeValues } : null, ConditionExpression ? { ConditionExpression } : null, capacity ? { ReturnConsumedCapacity: capacity.toUpperCase() } : null, metrics ? { ReturnItemCollectionMetrics: metrics.toUpperCase() } : null, returnValues ? { ReturnValues: returnValues.toUpperCase() } : null, typeof params === 'object' ? params : {});
        return payload;
    }
    /**
    ...
    updateTransaction(item, options = {}, params) {
        ...
        // Error on extraneous arguments
        if (Object.keys(args).length > 0) {
            (0, utils_js_1.error)(`Invalid update transaction options: ${Object.keys(args).join(', ')}`);
        }
    ...
    updateParams(item, options = {}, { SET = [], REMOVE = [], ADD = [], DELETE = [], ExpressionAttributeNames = {}, ExpressionAttributeValues = {}, ...params } = {}) {
        ...
        if (!Array.isArray(SET))
            (0, utils_js_1.error)('SET must be an array');
        if (!Array.isArray(REMOVE))
            (0, utils_js_1.error)('REMOVE must be an array');
        if (!Array.isArray(ADD))
            (0, utils_js_1.error)('ADD must be an array');
        if (!Array.isArray(DELETE))
            (0, utils_js_1.error)('DELETE must be an array');
        ...
        if ((ExpressionAttributeNames === null || ExpressionAttributeNames === void 0 ? void 0 : ExpressionAttributeNames.constructor) !== Object) {
            (0, utils_js_1.error)('ExpressionAttributeNames must be an object');
        }
        if ((ExpressionAttributeValues === null || ExpressionAttributeValues === void 0 ? void 0 : ExpressionAttributeValues.constructor) !== Object) {
            (0, utils_js_1.error)('ExpressionAttributeValues must be an object');
        }
        ...
        // Merge defaults
        const data = (0, normalizeData_js_1.default)()(schema.attributes, linked, Object.assign({}, defaults, item), shouldFilterUnmappedFields);
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid update options: ${args.join(', ')}`);
        // Verify metrics
        if (metrics !== undefined &&
            (typeof metrics !== 'string' || !['NONE', 'SIZE'].includes(metrics.toUpperCase()))) {
            (0, utils_js_1.error)(`'metrics' must be one of 'NONE' OR 'SIZE'`);
        ...
        if (returnValues !== undefined &&
            ...
            (0, utils_js_1.error)(`'returnValues' must be one of 'NONE', 'ALL_OLD', 'UPDATED_OLD', 'ALL_NEW', OR 'UPDATED_NEW'`);
        }
        let ConditionExpression; // init ConditionExpression
        // If conditions
        if (conditions) {
        ...
        Object.keys(required).forEach(field => required[field] &&
            ...
            (0, utils_js_1.error)(`'${field}${this.schema.attributes[field].alias ? `/${this.schema.attributes[field].alias}` : ''}' is a required field`));
        // Get partition and sort keys
        const Key = (0, getKey_js_1.default)()(data, schema.attributes, schema.keys.partitionKey, schema.keys.sortKey);
            ...
            if (field === '$remove') {
                ...
                for (const i in attrs) {
                    ...
                    if (!schema.attributes[attrs[i]]) {
                        (0, utils_js_1.error)(`'${attrs[i]}' is not a valid attribute and cannot be removed`);
                    }
                    // Verify attribute is not a pk/sk
                    if (schema.attributes[attrs[i]].partitionKey === true ||
                        schema.attributes[attrs[i]].sortKey === true) {
                        (0, utils_js_1.error)(`'${attrs[i]}' is the ${schema.attributes[attrs[i]].partitionKey === true ? 'partitionKey' : 'sortKey'} and cannot be removed`);
                    }
                    // Verify attribute is not required
                    if (schema.attributes[attrs[i]].required) {
                        (0, utils_js_1.error)(`'${attrs[i]}' is required and cannot be removed`);
                    }
                    const attributeHasDefaultValue = schema.attributes[attrs[i]].default !== undefined;
                    if (attributeHasDefaultValue) {
                        (0, utils_js_1.error)(`'${attrs[i]}' has a default value and cannot be removed`);
                    }
                ...
                if (schema.attributes[field].required)
                    (0, utils_js_1.error)(`'${field}' is required and cannot be removed`);
                REMOVE.push(`#${field}`);
                names[`#${field}`] = field;
            }
            else if (
            ...
            mapping.partitionKey !== true &&
                ...
                if (['bigint', 'number', 'set'].includes(mapping.type) &&
                    ...
                    data[field].$remove.forEach((i) => {
                        if (typeof i !== 'number') {
                            (0, utils_js_1.error)(`Remove array for '${field}' must only contain numeric indexes`);
                        }
                        REMOVE.push(`#${field}[${i}]`);
                    });
                    // Add field to names
                    ...
                    if (data[field].$append) {
                        SET.push(`#${field} = list_append(if_not_exists(#${field}, :${constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_KEY}) ,:${field})`);
                        values[`:${field}`] = validateType(mapping, field, data[field].$append);
                    }
                    else {
                        SET.push(`#${field} = list_append(:${field}, if_not_exists(#${field}, :${constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_KEY}))`);
                        values[`:${field}`] = validateType(mapping, field, data[field].$prepend);
                    }
                    // Add field to names
                    names[`#${field}`] = field;
                    values[`:${constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_KEY}`] = constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_VALUE;
                    ...
                    Object.keys(data[field]).forEach(i => {
                        if (String(parseInt(i)) !== i) {
                            (0, utils_js_1.error)(`Properties must be numeric to update specific list items in '${field}'`);
                        }
                        ...
                        props.forEach((prop, i) => {
                            ...
                            if (i === props.length - 1) {
                                ...
                                if (input === undefined || input === null) {
                                    ...
                                    SET.push(`${path} = list_append(if_not_exists(${path}, :${constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_KEY}), :${value})`);
                                    values[`:${value}`] = input.$append;
                                    // add default list value
                                    values[`:${constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_KEY}`] = constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_VALUE;
                                }
                                else if (input.$prepend) {
                                    SET.push(`${path} = list_append(:${value}, if_not_exists(${path}, :${constants_js_1.ATTRIBUTE_VALUES_LIST_DEFAULT_KEY}))`);
                    ...
                    const value = (0, utils_js_1.transformAttr)(mapping, validateType(mapping, field, data[field]), data);
        ...
        }, typeof params === 'object' ? params : {}, !(0, utils_js_1.isEmpty)(ExpressionAttributeValues) ? { ExpressionAttributeValues } : {}, ConditionExpression ? { ConditionExpression } : {}, capacity ? { ReturnConsumedCapacity: capacity.toUpperCase() } : null, metrics ? { ReturnItemCollectionMetrics: metrics.toUpperCase() } : null, returnValues ? { ReturnValues: returnValues.toUpperCase() } : null);
        return payload;
    ...
    putTransaction(item, options = {}, params) {
        ...
        // Error on extraneous arguments
        if (Object.keys(args).length > 0) {
            (0, utils_js_1.error)(`Invalid put transaction options: ${Object.keys(args).join(', ')}`);
        }
    ...
    putParams(item, options = {}, params = {}) {
        ...
        // Error on extraneous arguments
        if (args.length > 0)
            (0, utils_js_1.error)(`Invalid put options: ${args.join(', ')}`);
        // Verify metrics
        if (metrics !== undefined &&
            (typeof metrics !== 'string' || !['NONE', 'SIZE'].includes(metrics.toUpperCase()))) {
            (0, utils_js_1.error)(`'metrics' must be one of 'NONE' OR 'SIZE'`);
        ...
        (0, getKey_js_1.default)()(data, schema.attributes, schema.keys.partitionKey, schema.keys.sortKey);
        // Generate the payload
        const payload = Object.assign({
            TableName: _table.name,
            // Loop through valid fields and add appropriate action
            Item: Object.keys(data).reduce((acc, field) => {
                ...
                if (value !== undefined &&
                    ...
                    value = (0, utils_js_1.transformAttr)(mapping, value, data);
    ...
    conditionCheck(item, options = {}) {
        ...
        // Error on extraneous arguments
        if (Object.keys(args).length > 0) {
            (0, utils_js_1.error)(`Invalid conditionCheck options: ${Object.keys(args).join(', ')}`);
        }
        ...
        if (!('ConditionExpression' in payload))
            (0, utils_js_1.error)(`'conditions' are required in a conditionCheck`);
        // If ReturnValues exists, replace with ReturnValuesOnConditionCheckFailure
        if ('ReturnValues' in payload) {
            const { ReturnValues, ..._payload } = payload;
            payload = Object.assign({}, _payload, { ReturnValuesOnConditionCheckFailure: ReturnValues });
    ...
    setTable(table) {
        ...
        if (table != null && !((_b = table === null || table === void 0 ? void 0 : table.Table) === null || _b === void 0 ? void 0 : _b.attributes)) {
            (0, utils_js_1.error)(`Entity ${this.name} was assigned an invalid table`);
        }
